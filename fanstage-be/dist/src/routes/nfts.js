...
nftsRouter.get('/user', authMiddleware, async (c) => {
    try {
        ...
        const membershipPassList = await db
            ...
            name: sql `CONCAT(${users.username}, ' - ', ${membershipPasses.passTier}, ' Pass')`,
            description: sql `CONCAT('Official ', ${membershipPasses.passTier}, ' membership pass for ', ${users.username})`,
            image: sql `'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=400'`, // Mock image
            type: sql `'member-pass'`,
        ...
        const investmentNFTs = await db
            ...
            name: sql `CONCAT(${users.username}, ' - ', ${campaigns.projectTitle}, ' Investment')`,
            description: sql `CONCAT('Investment NFT for ', ${campaigns.projectTitle}, ' campaign')`,
            image: sql `'https://images.unsplash.com/photo-1614680376573-df3480f0c6ff?w=400'`, // Mock image
            type: sql `'proof-of-support'`,
...
nftsRouter.get('/campaign/:id', async (c) => {
    try {
        ...
        END`,
            ...
            investorName: sql `(SELECT username FROM users WHERE wallet_address = ${investments.investorId})`,
        })
            .from(investments)
            .leftJoin(campaigns, eq(investments.campaignId, campaigns.id))
            .leftJoin(users, eq(campaigns.artistId, users.walletAddress))
...
nftsRouter.post('/membership-pass', authMiddleware, async (c) => {
    try {
        ...
        const [existingPass] = await db.select()
            .from(membershipPasses)
            .where(and(eq(membershipPasses.ownerId, user.walletAddress), eq(membershipPasses.artistId, validatedData.artistId)))
            .limit(1);
        if (existingPass) {
