  ...
  try {
    ...
    const [newInvestment] = await db.insert(investments)
      .values({
        ...
        estimatedProfit: sql<number>`(${validatedData.investedAmountToken} * ${campaign.profitSharePercentage}) / 100`,
      })
      .returning();
    ...
    await db.update(campaigns)
      .set({
        currentFundingToken: sql<number>`current_funding_token + ${validatedData.investedAmountToken}`,
      })
      .where(eq(campaigns.id, validatedData.campaignId));
  ...
  try {
    ...
    const [investment] = await db
      ...
      .where(and(
        ...
        eq(investments.investorId, user.walletAddress)
      ))
      .limit(1);
