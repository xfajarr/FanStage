    ...
    try {
      ...
      if (privyUser.linkedAccounts && privyUser.linkedAccounts.length > 0) {
        const walletAccount = privyUser.linkedAccounts.find(
          (account: any) => account.type === 'wallet' && (account as any).address
        );
        
        if (walletAccount && (walletAccount as any).address) {
          console.log('💳 Found real wallet address:', (walletAccount as any).address);
          
          // Update user in database
          const [updatedUser] = await db
            .update(users)
            .set({ walletAddress: (walletAccount as any).address })
