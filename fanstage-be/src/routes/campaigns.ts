  ...
  try {
    ...
    let query = db
      .select({
        ...
        category: sql<string>`'album'`, // Default category, should be added to schema
        fundingGoal: campaigns.targetFundingToken,
        currentFunding: campaigns.currentFundingToken,
        backerCount: sql<number>`count(${investments.id})`,
        startDate: campaigns.createdAt,
        endDate: campaigns.deadline,
        profitShare: sql<{ fan: number; artist: number }>`json_build_object('fan', ${campaigns.profitSharePercentage}, 'artist', 100 - ${campaigns.profitSharePercentage})`,
    ...
    if (status) {
      conditions.push(eq(campaigns.campaignStatus, status as any));
    }
...
campaignsRouter.get('/featured', async (c) => {
  try {
    const featuredCampaigns = await db
      ...
      .leftJoin(investments, eq(campaigns.id, investments.campaignId))
      .where(and(
        eq(campaigns.campaignStatus, 'ongoing'),
        gte(campaigns.deadline, new Date())
      ))
      .groupBy(campaigns.id, users.username, users.profileImagUrl)
  ...
  try {
    ...
    const [existingCampaign] = await db.select()
      ...
      .where(and(
        ...
        eq(campaigns.artistId, user.walletAddress)
      ))
      .limit(1);
